// Zadanie 2 Lab4.cpp : Defines the entry point for the console application.
//
#include "stdafx.h"
#include <GL/glut.h>
#include "glext.h"
...
void MouseMotion(int x, int y)
{
	...
	{
		ry += 30 * (right - left) / glutGet(GLUT_WINDOW_WIDTH) * (x - button_x);
		button_x = x;
		rx -= 30 * (top - bottom) / glutGet(GLUT_WINDOW_HEIGHT) * (button_y - y);
		button_y = y;
		glutPostRedisplay();
	}
}
...
void GenerateTextures()
{
	...
	if (error == GL_FALSE)
	{
		printf("Niepoprawny odczyt pliku trawa.tga");
		exit(0);
	}
...
int main(int argc, char *argv[])
{
	...
	glutCreateWindow("Zadanie 2 Lab4");
	...
	glutAddMenuEntry("Test kanału alfa: wł/wył", GL_ALPHA_TEST);
	glutAddMenuEntry("Mieszanie kolorów: wł/wył", GL_BLEND);
	glutAddSubMenu("Aspekt obrazu", MenuAspect);
	glutAddMenuEntry("Wyjście z Programu", EXIT);
#else

	glutAddMenuEntry("Test kanalu alfa: wl/wył", GL_ALPHA_TEST);
	glutAddMenuEntry("Mieszanie kolorow: wł/wył", GL_BLEND);
	glutAddSubMenu("Aspekt obrazu", MenuAspect);
	glutAddMenuEntry("Wyjscie z Programu", EXIT);
#endif
